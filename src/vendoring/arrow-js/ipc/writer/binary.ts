...
export class TypeSerializer extends TypeVisitor {
    ...
    public visitDecimal(node: Decimal) {
        ...
        return _Decimal.endDecimal(b);
    }
    public visitDate(node: Date_) {
        const b = this.builder;
        _Date.startDate(b);
        _Date.addUnit(b, node.unit);
        return _Date.endDate(b);
    }
    ...
    public visitTimestamp(node: Timestamp) {
        ...
        if (timezone !== undefined) { _Timestamp.addTimezone(b, timezone); }
        return _Timestamp.endTimestamp(b);
    }
    public visitInterval(node: Interval) {
        const b = this.builder;
        _Interval.startInterval(b);
        _Interval.addUnit(b, node.unit);
        return _Interval.endInterval(b);
    }
    public visitList(_node: List) {
        const b = this.builder;
        _List.startList(b);
        return _List.endList(b);
    }
    public visitStruct(_node: Struct) {
        const b = this.builder;
        _Struct.startStruct_(b);
        return _Struct.endStruct_(b);
    }
    public visitUnion(node: Union) {
        const b = this.builder;
        _Union.startTypeIdsVector(b, node.typeIds.length);
        const typeIds = _Union.createTypeIdsVector(b, node.typeIds);
        _Union.startUnion(b);
        _Union.addMode(b, node.mode);
        _Union.addTypeIds(b, typeIds);
        return _Union.endUnion(b);
    }
    public visitDictionary(node: Dictionary) {
        ...
        if (indexType !== undefined) { _DictionaryEncoding.addIndexType(b, indexType); }
        return _DictionaryEncoding.endDictionaryEncoding(b);
    }
    public visitFixedSizeBinary(node: FixedSizeBinary) {
        ...
        return _FixedSizeBinary.endFixedSizeBinary(b);
    }
...
function writeFooter(b: Builder, node: Footer) {
    ...
    let recordBatchesOffset = b.endVector();
...
function writeSchema(b: Builder, node: Schema) {
    ...
    const fieldsOffset = _Schema.createFieldsVector(b, fieldOffsets);
    ...
    if (metadata !== undefined) { _Schema.addCustomMetadata(b, metadata); }
...
function writeRecordBatch(b: Builder, node: RecordBatchMetadata) {
    ...
    let nodesOffset = mapReverse(nodes, (n) => writeFieldNode(b, n)) &&
        b.endVector();

    _RecordBatch.startBuffersVector(b, buffers.length);
    let buffersOffset = mapReverse(buffers, (b_) => writeBuffer(b, b_)) &&
        b.endVector();
...
function writeField(b: Builder, node: Field) {
    ...
    if (name !== undefined) {
        _Field.addName(b, name);
    }
    if (dictionary !== undefined) {
        _Field.addDictionary(b, dictionary);
    }
    if (metadata !== undefined) {
        _Field.addCustomMetadata(b, metadata);
    }
